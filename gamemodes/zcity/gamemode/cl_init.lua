...
function GM:ScoreboardShow()
	...
	for i, ply in player.Iterator() do -- надо это говно переделать.
		...
		-- Создаем контейнер для аватара с рамкой
		local avatarContainer = vgui.Create("DPanel", but)
		avatarContainer:Dock(LEFT)
		avatarContainer:SetText("")
		avatarContainer:SetPaintBackground(false)
		
		local avatarSize = math.Clamp( math.floor( ScrH() * 0.04 ), 24, 96 )
		avatarContainer:SetSize(avatarSize, avatarSize)
		
		local marginX = math.max(4, math.floor( avatarSize * 0.25 ))
		local marginY = math.max(3, math.floor( avatarSize * 0.18 ))
		avatarContainer:DockMargin( marginX, marginY, marginX, marginY )
		
		-- PaintOver функция для контейнера (черная рамка поверх всего)
		function avatarContainer:PaintOver(w, h)
			-- Черная рамка вокруг аватара
		...
		avatar:SetPos(0, 0)
		avatar:SetPlayer(ply, math.Clamp( avatarSize * 2, 64, 128 ))
		...
		but.Paint = function(self, w, h)
			...
			surface.SetTextPos(80, h / 2 - lengthY / 2)
...
-- Отступы тоже масштабируются под размер аватара
local marginX = math.max(4, math.floor( avatarSize * 0.25 ))
local marginY = math.max(3, math.floor( avatarSize * 0.18 ))
avatar:DockMargin( marginX, marginY, marginX, marginY )

-- Второй параметр SetPlayer — размер текстуры аватара (рекомендуется >= avatarSize)
avatar:SetPlayer(ply, math.Clamp( avatarSize * 2, 64, 128 ))
		...
		but.Paint = function(self,w,h)
			...
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize( ply:Ping() or "He quited..." )
			surface.SetTextPos(w - lengthX -35,h/2 - lengthY/2)
			surface.DrawText(ply:Ping() or "He quited...")
		end
